...
def _validate_parent_child_relations(tree: LineStatsTree):
    for line in tree.lines.values():
        if line.parent_key is None:
            continue
        assert line.parent_key in tree.lines, f"Parent key {line.parent_key} not found in tree"
        assert line.key in tree.lines[line.parent_key].child_keys, f"Line {line.key} should have parent key {line.parent_key}"
        
def _validate_children_time_sum(tree: LineStatsTree):
    for line in tree.lines.values():
        if line.parent_key is None:
            continue
        sum_children_time = sum(tree.lines[child_key].time for child_key in line.child_keys)
        assert line.child_time - sum_children_time < 1e-6, f"Line {line.key} should have child time {line.child_time} but sum of children time {sum_children_time} is not equal"
...
def _validate_time_sleep(tree: LineStatsTree):
    for line in tree.lines.values():
        if "time.sleep" in line.source:
            ...
            assert line.time == pytest.approx(total_time, rel=0.1), f"Line {line.key} should have time {total_time} but has time {line.time}"
